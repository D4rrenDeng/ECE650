    ...
    for key, values in vertex_position_index.items():
        ...
        for i in range(len(values) - 1):
            ...
            if not (vx1, vy1) in vertices:
                vertices[(vx1, vy1)] = vertex_count
                vertex_count += 1
            if not (vx2, vy2) in vertices:
                vertices[(vx2, vy2)] = vertex_count
                vertex_count += 1
            edges.append((vertices[(vx1, vy1)], vertices[(vx2, vy2)]))
    print("V " + str(vertex_count), flush = True)
    for i in range(len(edges)):
        if i < len(edges) - 1:
            output = output + "<" + str(edges[i][0]) + "," + str(edges[i][1]) + ">,"
        else:
            output = output + "<" + str(edges[i][0]) + "," + str(edges[i][1]) + ">}"
    ...
    while True:
        try:
            ...
            if len(line_list) == 3:
                ...
                # check street name
                if re.match("^[a-zA-Z0-9 ]+$", street_name):
                    pass
                else:
                    print("Error: illegal streetname!", flush = True)
                    continue

                if(command[len(command) - 1] != ' '):
                    print("Error: missing space between command!", flush = True)
                    continue
                ...
                if command == 'r':
                    points.replace(' ', '')
                    if points != '':
                        print("Error: unrecognized command!", flush = True)
                        continue
                    if street_name in network:
                        ...
                        print("Error: street not found!", flush = True)
                    continue

                if (points == ''):
                    print("Error: empty coordinates!", flush = True)
                    continue
                # add or change
                if points[0] != ' ':
                    print("Error: missing space between command!", flush = True)
                ...
                if command == 'a':
                    if (not check_pattern(points)):
                        print("Error: coordinates were in the wrong format!", flush = True)
                        continue
                    # add the street to the network
                    if street_name in network:
                        print("Error: street already exists!", flush = True)
                        continue
                    network[street_name] = []
